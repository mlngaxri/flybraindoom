...
const files=['app.js','game.js','policy.js','brain-worker.js'];for(const file of files)execFileSync(process.execPath,['--check',file],{stdio:'inherit'});JSON.parse(fs.readFileSync('vercel.json','utf8'));
const html=fs.readFileSync('index.html','utf8'),app=fs.readFileSync('app.js','utf8'),game=fs.readFileSync('game.js','utf8'),policy=fs.readFileSync('policy.js','utf8'),worker=fs.readFileSync('brain-worker.js','utf8');
for(const id of new Set([...app.matchAll(/\$\('#([^']+)'\)/g)].map(m=>m[1])))if(!html.includes(`id="${id}"`))throw new Error(`Missing DOM id: ${id}`);
...
if(!game.includes('drawAvatar()')||!game.includes('c.arc(cx-17'))throw new Error('Smiling cube avatar is missing.');
...
if(!app.includes("EXPERIMENT_KEY='flybrainsaber.experiment.v1'"))throw new Error('Saber experiment persistence key is missing.');
if(!policy.includes("POLICY_KEY = 'flybrainsaber.policy.v1'"))throw new Error('Saber learner persistence key is missing.');
...
const {ACTIONS}=await import(pathToFileURL(`${process.cwd()}/game.js`));if(ACTIONS.map(a=>a.name).join(',')!=='idle,left-swing,right-swing')throw new Error('Unexpected action ordering.');
const {LinearQLearner,neuralFeatures}=await import(pathToFileURL(`${process.cwd()}/policy.js`));const x=neuralFeatures({active_neurons:1200,mean_rate_hz:4.2,channels:{turn_bias:.2,escape_takeoff:.1,escape_long_mode:.03,stop_freeze:.02,backward_walk:.01},dn_rates:{DN_left:12,DN_right:19,DN_escape:8},proboscis_drive:0});if(x.length!==14||[...x].some(v=>!Number.isFinite(v)))throw new Error('Neural feature vector is invalid.');const learner=new LinearQLearner(14,3);learner.update(x,1,5,x,false);if(learner.updates!==1||learner.weights[1].some(v=>!Number.isFinite(v)))throw new Error('Learner update failed.');console.log('verify: saber build checks passed');
